...
describe('Order Handler', () => {
    ...
    describe('No orders', () => {
        it(`Order Requirement 1 GET /orders/current test - No Token
        ...
        it(`Order Requirement 1 GET /orders/current test - with Token
    ...
    describe('Has orders', () => {
        ...
        it(`Order Requirement 1 GET /orders/current test - with Token
            ...
            try {
                ...
                expect(parseInt(result.body.id)).toEqual(
                    parseInt(active_order_id)
                );
                expect(parseInt(result.body.user_id)).toEqual(
                    parseInt(user_id)
                );
            } catch (err) {
                console.log(err);
        ...
        it(`Order Requirement 2 GET /orders/completed test - with Token
            ...
            try {
                ...
                expect(result.body.order[0].id).toEqual(
                    parseInt(completed_order_id)
                );
            } catch (err) {
                console.log(err);
                expect(err).toBe('');
